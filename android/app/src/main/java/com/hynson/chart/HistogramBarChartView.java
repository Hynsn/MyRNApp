 ...
 * 描述：镂空柱状图
 */
public class HistogramBarChartView extends BaseChartView {
    public HistogramBarChartView(Context context) {
        super(context);
        init();
    }

    public HistogramBarChartView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public HistogramBarChartView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init();
    }

    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    public HistogramBarChartView(Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init() {
        notMedicineDistance = dip2px(getContext(), 20);
        medicineDistance = dip2px(getContext(), 75);
    ...
    private int mNoMedicineColor = pinkColor;//未服药颜色
    private int mMedicineColor = pinkLightColor;//服药颜色
    ...
    private boolean isShowTopInfo = true;//是否显示顶部信息
    private boolean isShowYCenterText = false;//是否显示Y轴中间文本

    public HistogramBarChartView setShowTopInfo(boolean showTopInfo) {
        isShowTopInfo = showTopInfo;
        return this;
    }

    public HistogramBarChartView setShowYCenterText(boolean showYCenterText) {
        isShowYCenterText = showYCenterText;
        return this;
    }

    public HistogramBarChartView setNoMedicineColor(int mNoMedicineColor) {
        this.mNoMedicineColor = mNoMedicineColor;
        return this;
    }

    public HistogramBarChartView setMedicineColor(int mMedicineColor) {
        this.mMedicineColor = mMedicineColor;
        return this;
    }

    public HistogramBarChartView setNoMedicineText(String mNoMedicineText) {
        this.mNoMedicineText = mNoMedicineText;
        return this;
    }

    public HistogramBarChartView setMedicineText(String mMedicineText) {
        this.mMedicineText = mMedicineText;
        return this;
    }

    public HistogramBarChartView setBloodText(String mBloodText) {
        this.mBloodText = mBloodText;
        return this;
    }

    public HistogramBarChartView setPillarWidth(float pillarWidth) {
...
//                }
                if (isShowYCenterText) {
                    canvas.drawText(y_scale_text[i], yBottomPoint.x, yScale[i].y, blackPaint);
                }else{
                    if (i == 2) {
                        //血压这里，最后第二位不显示
                        canvas.drawText("", yBottomPoint.x, yScale[i].y, blackPaint);
                    }else{
                        canvas.drawText(y_scale_text[i], yBottomPoint.x, yScale[i].y, blackPaint);
                    }
                }
